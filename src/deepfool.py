...
def deepfool(image, nets, num_classes=10, overshoot=0.02, max_iter=200):
    ...
    #Finding the class of the sample, assuming all nets label it the same 
    label = None
    f_image = nets[0].forward(image[None, :]).data.cpu().numpy().flatten()
    ...
    return r_tot, loop_i, label, most_common, pert_image, found 
